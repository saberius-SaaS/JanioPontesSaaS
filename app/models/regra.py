...
class RegraObrigacao(TenantMixin, Base):
    ...
    departamento = Column(String(50), nullable=True, comment="CONTABIL, FISCAL, PESSOAL, SOCIETARIO")
    regime = Column(String(50), nullable=True, comment="Filtro de regime tributário")
    acao = Column(String(50), nullable=True, comment="ENVIAR, ARQUIVAR, AUDITAR, COMUNICAR")
    meses = Column(String(100), nullable=True, comment="Meses aplicáveis (ex: '1,2,3...12')")
    tipos = Column(String(255), nullable=True, comment="Tipos de empresa aplicáveis")
    desloca = Column(Integer, nullable=True, default=0, comment="Deslocamento de mês-referência")
    vencimento_legal = Column(String(50), nullable=True, comment="Data legal de referência")
    antecipa_fds = Column(String(10), nullable=True, comment="S/N — antecipa se cair no fim de semana")
    grupo_regra = Column(String(100), nullable=True, comment="Agrupamento/Tag de regras")
